...
class XBizSpider(scrapy.Spider):
	...
	def parse(self, response):
		"""For parsing the main list of buissness which is returned by the search reqest"""

		# parsing SOURCE_ID's from main list of buissness entites
		dirty_ids = response.selector.re('"ID":\d+')
		clean_ids = [x.split(':')[1] for x in dirty_ids]
		res_json = json.loads(response.text) #main list of buissnes to json
		biz_title = {}

		for id in clean_ids:
			# Store ID & Title
			biz_title[id] = res_json["rows"][id]['TITLE'][0] 
			print(res_json["rows"][id]['TITLE'][0] )
			# Request Details
	...
	def parse_biz(self, response):
		"""For parsing indivdual buissness' information"""
		maddy_xp=\
		...
		#
		# pickle.dump({id:{'maddress': maddress,
		# 				 'owner': owner,
		# 				 'agent': agent,
		# 				 }
		# 			},
